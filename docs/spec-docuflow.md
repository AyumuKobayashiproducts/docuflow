...
## 🏢 組織・権限モデル（Team / Organization）

### 目的

- 個人利用だけでなく、**チーム単位でのドキュメント共有・検索** を安全に行えるようにする。
- 将来的な **有料プラン（組織課金）** を見据えたマルチテナント設計を採用する。

### 概念モデル

- **Organization（組織）**
  - 企業 / プロジェクトチーム単位のコンテナ。
  - 1ユーザーは複数の Organization に所属可能。
- **Member（組織メンバー）**
  - `organization_members` テーブルで表現。
  - 各メンバーには `role`（`owner` / `admin` / `member`）が付与される。
- **Document（ドキュメント）**
  - `documents.organization_id` を持つと「組織ドキュメント」、NULL の場合は「個人ドキュメント」として扱う。

### ロールと権限

| ロール   | 説明                               | 主な権限                                                                 |
|:--------|:-----------------------------------|:-------------------------------------------------------------------------|
| owner   | 組織のオーナー                     | 組織設定変更 / メンバー追加・削除 / 組織ドキュメントの削除               |
| admin   | 組織管理者                         | メンバーのロール変更 / 組織ドキュメントの作成・編集・削除               |
| member  | 組織メンバー（一般ユーザー）       | 組織ドキュメントの閲覧・作成・編集（削除は制限する場合も検討）         |
...
- `organization_invitations`
  - `id`, `organization_id`, `email`, `role`, `token`, `expires_at`, `accepted_at`, `created_at`
- 既存テーブルの拡張
  - `documents.organization_id`
  - `activity_logs.organization_id`
  - `document_comments.organization_id`
...
- `document_comments`
  - 対象ドキュメントにアクセス権があるユーザーのみ、コメントの閲覧・作成が可能。
- `organizations` / `organization_members` / `organization_invitations`
  - 組織メンバーは所属組織の情報のみ参照可能。
  - 管理操作（メンバー追加・ロール変更・招待発行など）は `owner` のみ実行可能。

### UI / UX 要件

- ヘッダーに **Organization スイッチャー** を配置
  - 現在選択中の組織名を表示
  - ドロップダウンから所属している組織を切り替え
  - 「個人ワークスペース」に戻るオプション
- `/settings/organizations` 画面
  - 所属組織一覧（名前 / ロール / メンバー数）
  - 組織の新規作成（`owner` で作成）
  - メンバー一覧とロール変更（`owner` / `admin`）
  - 招待リンクorメールアドレスでの招待管理（`organization_invitations`）

### 将来拡張

- 組織ごとの **プラン / シート数** の管理
- 組織別の AI 利用制限（リクエスト数 / 日）とメータリング
- 組織単位の監査ログエクスポート（CSV / BigQuery 連携 など）
...
#### 4.3.1 AI ベクトル検索（pgvector）

- ドキュメント保存時に OpenAI `text-embedding-3-small` で 1536次元の埋め込みベクトルを生成・保存する。
- ユーザーの検索クエリもベクトル化し、コサイン類似度で意味的に近いドキュメントを検索する。
- `/app` の検索結果に「AI類似検索結果」セクションとして表示する。
- 類似度閾値（デフォルト 0.7）以上のドキュメントのみ返す。
- 技術スタック:
  - Supabase pgvector 拡張
  - `match_documents` RPC 関数
  - IVFFlat インデックス（lists=100）
...
| 通知タイプ | トリガー | 説明 |
|:-----------|:---------|:-----|
| `comment_added` | ドキュメントにコメント追加 | ドキュメントのオーナーに通知 |
| `comment_mention` | コメント内で@メンション | メンションされたユーザーに通知 |
| `share_link_created` | 共有リンク発行 | アクティビティとして記録 |
| `document_shared` | ドキュメント共有 | 共有先ユーザーに通知 |
| `org_invitation` | 組織への招待 | 招待されたユーザーに通知 |
| `org_member_joined` | 組織に新メンバー参加 | 組織のowner/adminに通知 |
| `document_updated` | ウォッチ中のドキュメント更新 | ウォッチャーに通知 |

#### 6.2 通知の配信方法

| 方法 | 実装状況 | 説明 |
|:-----|:---------|:-----|
| アプリ内通知 | ✅ 実装済み | ヘッダーの🔔アイコンからドロップダウン表示 |
| メール通知 | 📋 設計のみ | SendGrid / Resend での実装を想定 |
| プッシュ通知 | 📋 将来対応 | PWA の Web Push API を想定 |

#### 6.3 メンション機能

- コメント本文中の `@メールアドレス` または `@名前` を検出
- メンションされたユーザーに `comment_mention` 通知を送信
- `document_comments.mentioned_user_ids` に解決済みユーザーIDを保存

#### 6.4 非機能要件（通知）

| 項目 | 目標値 |
|:-----|:-------|
| 通知配信遅延 | アプリ内: リアルタイム（ページリロード時） |
| 未読管理 | `read_at` で個別管理、一括既読も可能 |
| 保持期間 | 無期限（将来的に90日でアーカイブ検討） |

### 7. 制約・スコープ外

- ~~本実装では組織・チーム単位のワークスペース管理は行わない（単一ユーザー前提）。~~ → 🏢 組織機能を実装済み
- ~~高度なアクセス制御（ロールベース / 共有先ユーザーの限定）はスコープ外。~~ → RBACを実装済み
- RLS はポリシーのみ定義し、ローカル開発では disabled とする。
- リアルタイム同期（Supabase Realtime）は将来対応。
- メール通知の送信キュー / リトライ機構は将来対応。
