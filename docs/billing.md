...
### 1.1 プラン一覧（価格は Stripe の Price を正とする）

| Plan | 対象 | 主な制限 | 価格 |
|:-----|:-----|:---------|:----------|
| `free` | 個人 / お試し | ドキュメント 50件 / ストレージ 100MB / AI呼び出し 100回/月 / メンバー 1人 | 無料 |
| `pro` | 小規模チーム / 個人 | ドキュメント 1,000件 / ストレージ 5GB / AI呼び出し 5,000回/月 / メンバー 10人 | Stripe で管理（`STRIPE_PRICE_PRO_MONTH`） |
| `team` | 中規模チーム | ドキュメント 10,000件 / ストレージ 50GB / AI呼び出し 50,000回/月 / メンバー 50人 | Stripe で管理（`STRIPE_PRICE_TEAM_MONTH`） |
| `enterprise` | 大規模組織 | 無制限 / カスタムブランディング / API アクセス / 優先サポート | Stripe で管理（`STRIPE_PRICE_ENTERPRISE_MONTH`）またはお問い合わせ |
