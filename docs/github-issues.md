...
## 実装（現在の実装方針）
- [x] **マーケサイトはルート分離**: `/`（日本語）と `/en`（英語）
- [x] **アプリ本体は `?lang=` で切替**: `?lang=en` / `?lang=ja`（URLがソース・オブ・トゥルース）
- [x] **言語の永続化**: `docuflow_lang` cookie（＋localStorage）で保存
- [x] **自動判定**: cookie優先 → cookie無ければ `Accept-Language`（`ja`以外は英語）
- [x] **英語ユーザーの迷子防止**:
  - `/` → `/en`（英語ユーザー）
  - `/pricing` など日本語ページ直リンクでも `/en/*` へ誘導
- [x] **共通UIの英語化**: ヘッダー/サイドバー/フィルタ/カード/アクティビティ等で英語時に日本語が混ざらない
...
## 参考
- （補足）将来、言語数が増える場合は `next-intl` 等の導入も検討余地あり
```
