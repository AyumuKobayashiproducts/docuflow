...
export function SearchFilter({
  ...
  const quickFilters = [
    { key: "all", label: locale === "en" ? "All" : "すべて", active: !selectedCategory && !onlyPinned && !onlyFavorites && !showArchived },
    { key: "pinned", label: locale === "en" ? "Pinned" : "ピン留め", active: onlyPinned },
    { key: "favorites", label: locale === "en" ? "Favorites" : "お気に入り", active: onlyFavorites },
    { key: "archived", label: locale === "en" ? "Archived" : "アーカイブ", active: showArchived },
  ];
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          <span className="hidden sm:inline">{locale === "en" ? "Filters" : "フィルター"}</span>
        ...
        <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 space-y-4 animate-fade-in">
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              >
                <option value="">{locale === "en" ? "All categories" : "すべて"}</option>
                {categories.map((cat) => (
